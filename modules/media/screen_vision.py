...
def _save_config_key(key: str, value) -> None:
    try:
        ...
        print(f"[ScreenAI] ⚠️  Could not save config key '{key}': {e}")
...
_GEMINI_MODEL_ID         = "models/gemini-2.5-flash-native-audio-preview-12-2025"
_CHANNELS           = 1
_SPEAKER_SAMPLE_HZ = 24_000
_AUDIO_FRAME_SIZE         = 1_024
...
_SYSTEM_PROMPT = (
    "You are LEO, an advanced AI assistant created by Varun Nayaka. "
...
def _compress(img_bytes: bytes, source_format: str = "PNG") -> tuple[bytes, str]:
    ...
    try:
        ...
        print(f"[ScreenAI] ⚠️  Image compress failed: {e}")
        return img_bytes, f"image/{source_format.lower()}"
...
def _detect_camera_index() -> int:
    ...
    print("[ScreenAI] 🔍 Auto-detecting camera...")
    for idx in range(6):
        if _probe_camera(idx, backend):
            print(f"[ScreenAI] ✅ Camera found at index {idx}")
            _save_config_key("camera_index", idx)
            return idx
        print(f"[ScreenAI] ⚠️  Camera index {idx}: no usable frame")

    print("[ScreenAI] ⚠️  No camera found — defaulting to index 0")
...
class _VisionSession:
    ...
    async def _session_loop(self) -> None:
        ...
        while True:
            try:
                print("[ScreenAI] 🔌 Connecting...")
                async with client.aio.live.connect(
                    model=_GEMINI_MODEL_ID, config=config
                ) as session:
                    self._session = session
                    self._ready_evt.set()
                    backoff = 2.0  
                    print("[ScreenAI] ✅ Connected")
                ...
                for exc in eg.exceptions:
                    print(f"[ScreenAI] ⚠️  Session error: {exc}")
            finally:
                self._session = None
                self._ready_evt.clear()

            print(f"[ScreenAI] 🔄 Reconnecting in {backoff:.0f}s...")
    ...
    async def _send_loop(self) -> None:
        while True:
            ...
            if not self._session:
                print("[ScreenAI] ⚠️  No session — dropping image")
                continue
            try:
                ...
                print(f"[ScreenAI] 📤 Sent {len(image_bytes):,} bytes — '{user_text[:60]}'")
            except Exception as e:
                print(f"[ScreenAI] ⚠️  Send error: {e}")
            ...
            async for response in self._session.receive():
                ...
                if sc.turn_complete:
                    if transcript and self._player:
                        ...
                        if full:
                            self._player.write_log(f"LEO: {full}")
                            print(f"[ScreenAI] 💬 {full}")
                    transcript = []

        except Exception as e:
            print(f"[ScreenAI] ⚠️  Recv error: {e}")
            raise  

    async def _play_loop(self) -> None:
        stream = sd.RawOutputStream(
            samplerate=_SPEAKER_SAMPLE_HZ,
            channels=_CHANNELS,
            dtype="int16",
            blocksize=_AUDIO_FRAME_SIZE,
        )
        stream.start()
        try:
            while True:
...
def analyze_display(
    ...
    if not user_text:
        print("[ScreenAI] ⚠️  No question provided — aborting")
        return False

    print(f"[ScreenAI] ▶ angle={angle!r}  question='{user_text[:80]}'")

    try:
        _ensure_session(player=player)
    except Exception as e:
        print(f"[ScreenAI] ❌ Could not start session: {e}")
        return False

    try:
        if angle == "camera":
            image_bytes, mime_type = _capture_camera()
            print(f"[ScreenAI] 📷 Camera: {len(image_bytes):,} bytes")
        else:
            image_bytes, mime_type = _capture_screen()
            print(f"[ScreenAI] 🖥️  Screen: {len(image_bytes):,} bytes")
    except Exception as e:
        print(f"[ScreenAI] ❌ Capture error: {e}")
        return False
...
def warmup_session(player=None) -> None:
    try:
        ...
        print(f"[ScreenAI] ⚠️  Warmup failed: {e}")
