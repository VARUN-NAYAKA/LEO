...
class AssistantInterface:
    def __init__(self, face_path, size=None):
        self.root = tk.Tk()
        self.root.title("L.E.O — Linguistic Executive Officer")
        ...
        self._current_mode = "INITIALISING"
    ...
    def _draw(self):
        ...
        for pr in self.pulse_r:
            ...
            if self.muted:
                ...
                c.create_oval(FCX-r, FCY-r, FCX+r, FCY+r,
                              outline=self._ac(124, 58, 237, pa), width=2)
        ...
        for idx, (r_frac, w_ring, arc_l, gap) in enumerate([
            ...
            col    = self._ac(239, 68, 68, a_val) if self.muted else self._ac(124, 58, 237, a_val)
        ...
        scan_col = self._ac(239, 68, 68, scan_a) if self.muted else self._ac(124, 58, 237, scan_a)
        ...
        c.create_arc(FCX-sr, FCY-sr, FCX+sr, FCY+sr,
                     ...
                     outline=self._ac(245, 158, 11, scan_a // 2), width=2, style="arc")

        t_out = int(FW * 0.495)
        t_in  = int(FW * 0.472)
        a_mk  = self._ac(124, 58, 237, 155)
        ...
        ch_r = int(FW * 0.50)
        gap  = int(FW * 0.15)
        ch_a = self._ac(124, 58, 237, int(self.halo_a * 0.55))
        for x1, y1, x2, y2 in [
        ...
        if self._has_face:
            ...
            orb_color = (239, 68, 68) if self.muted else (49, 27, 105)
            ...
            c.create_text(FCX, FCY, text=SYSTEM_NAME,
                          fill=self._ac(124, 58, 237, min(255, int(self.halo_a * 2))),
                          font=("Courier", 14, "bold"))

        HDR = 62
        c.create_rectangle(0, 0, W, HDR, fill="#0c0b1e", outline="")
        c.create_line(0, HDR, W, HDR, fill=C_MID, width=1)
        c.create_text(W // 2, 22, text=SYSTEM_NAME,
                      fill=C_PRI, font=("Courier", 18, "bold"))
        c.create_text(W // 2, 44, text="Linguistic Executive Officer",
        ...
        if self.muted:
            ...
            stat = "● SPEAKING"
            sc   = C_ACC
        elif self._current_mode == "THINKING":
        ...
        c.create_rectangle(0, H - 28, W, H, fill="#0c0b1e", outline="")
        ...
        c.create_text(W // 2, H - 14, fill=C_DIM, font=("Courier", 8),
                      text="Varun Nayaka  ·  AUTONOMOUS  ·  LEO")
    ...
    def _show_setup_ui(self):
        ...
        tk.Label(
            self.setup_frame,
            text="Configure L.E.O before first boot.",
            fg=C_MID, bg="#0c0b1e",
            font=("Courier", 9)
        ).pack(pady=(0, 14))
        ...
        sep_frame = tk.Frame(self.setup_frame, bg="#0c0b1e")
        ...
        os_btn_frame = tk.Frame(self.setup_frame, bg="#0c0b1e")
        ...
        sep_frame2 = tk.Frame(self.setup_frame, bg="#0c0b1e")
    ...
    def _select_os(self, os_key: str):
        ...
        styles = {
            "windows": (C_PRI,    "#1a1640"),
            "mac":     (C_ACC2,   "#1a1a00"),
            "linux":   (C_GREEN,  "#0d1a0d"),
        }
        for key, btn in self._os_buttons.items():
            if key == os_key:
                ...
                btn.configure(
                    fg=C_DIM, bg="#12112a",
                    activeforeground=C_TEXT, activebackground="#1a1640",
                    relief="flat"
                )
    ...
    def _save_api_keys(self):
        ...
        self.set_state("LISTENING")
        self.write_log(f"SYS: Systems initialised. OS → {os_system.upper()}. LEO online.")