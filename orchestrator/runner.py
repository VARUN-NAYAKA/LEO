...
from orchestrator.planner       import create_plan, replan
from orchestrator.recovery import analyze_error, generate_fix, ErrorDecision
...
def _run_generated_code(description: str, speak: Callable | None = None) -> str:
    ...
    try:
        ...
        print(f"[Runner] 🐍 Running generated code: {tmp_path}")
    ...
    if tool == "file_controller" and params.get("action") in ("write", "create_file"):
        ...
        if not content or len(content) < 50:
            ...
            if all_results:
                ...
                print(f"[Runner] 💉 Injected + translated content")
...
def _translate_to_goal_language(content: str, goal: str) -> str:
    ...
    try:
        ...
        print(f"[Runner] 🌐 Translating to: {target_lang}")
...
def _call_tool(tool: str, parameters: dict, speak: Callable | None) -> str:
    ...
    if tool == "open_app":
        ...
        from modules.data.flights import flight_finder
        return flight_finder(parameters=parameters, player=None, speak=speak) or "Done."

    else:
        print(f"[Runner] ⚠️ Unknown tool '{tool}' — falling back to generated_code")
    ...
    def execute(
        ...
        while True:
            ...
            for step in steps:
                ...
                print(f"\n[Runner] ▶️ Step {step_num}: [{tool}] {desc}")
                ...
                while attempt <= 3:
                    ...
                    try:
                        ...
                        print(f"[Runner] ✅ Step {step_num} done: {str(result)[:100]}")
                        step_ok = True
                        break

                    except Exception as e:
                        error_msg = str(e)
                        print(f"[Runner] ❌ Step {step_num} attempt {attempt} failed: {error_msg}")
                        ...
                        if decision == ErrorDecision.RETRY:
                            ...
                            print(f"[Runner] ⏭️ Skipping step {step_num}")
                            completed_steps.append(step)
                            step_ok = True
                            break
                            ...
                            if fix_suggestion and tool != "generated_code":
                                try:
                                    ...
                                    print(f"[Runner] ⚠️ Fix failed: {fix_err}")
