...
class LeoUI:
    ...
    def _draw(self):
        ...
        if self.muted:
            ...
            stat = "● SPEAKING"
            sc   = C_ACC
        elif self._LEO_state == "THINKING":
            sym  = "◈" if self.status_blink else "◇"
            stat = f"{sym} THINKING"
            sc   = C_ACC2
        elif self._LEO_state == "PROCESSING":
            sym  = "▷" if self.status_blink else "▶"
            stat = f"{sym} PROCESSING"
            sc   = C_ACC2
        elif self._LEO_state == "LISTENING":
            sym  = "●" if self.status_blink else "○"
        ...
        c.create_text(W // 2, H - 14, fill=C_DIM, font=("Courier", 8),
                      text="Varun Nayaka  ·  CLASSIFIED  ·  LEO")
