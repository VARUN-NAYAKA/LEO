...
ERROR_ANALYST_PROMPT = """You are the error recovery module of LEO AI assistant.
...
def analyze_error(
    ...
    if attempt >= max_attempts:
        print(f"[Recovery] ⚠️ Max attempts reached for step {step.get('step')} — forcing replan")
        return {
...
Attempt number: {attempt}"""
    ...
    try:
        ...
        print(f"[Recovery] Decision: {result['decision'].value} — {result.get('reason', '')}")
        return result

    except Exception as e:
        print(f"[Recovery] ⚠️ Analysis failed: {e} — defaulting to replan")
        return {
            "decision":       ErrorDecision.REPLAN,
            "reason":         str(e),
...
Return ONLY the Python code, no explanation."""
    ...
    try:
        ...
        print(f"[Recovery] ⚠️ Fix generation failed: {e}")
        return {
            "step":        step.get("step"),
            "tool":        "generated_code",
