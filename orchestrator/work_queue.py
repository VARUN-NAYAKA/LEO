...
class WorkQueue:
    ...
    def start(self) -> None:
        ...
        print("[WorkQueue] ✅ Started")

    def stop(self) -> None:
        self._running = False
        with self._condition:
            self._condition.notify_all()
        print("[WorkQueue] 🔴 Stopped")
    ...
    def submit(
        ...
        print(f"[WorkQueue] 📥 Task queued: [{task_id}] {goal[:60]}")
        return task_id
    ...
    def cancel(self, task_id: str) -> bool:
        ...
        with self._lock:
            ...
            print(f"[WorkQueue] 🚫 Task cancelled: [{task_id}]")
            return True
    ...
    def pending_count(self) -> int:
        with self._lock:
            return sum(1 for t in self._queue if t.status == JobStatus.PENDING)
    ...
    def _next_task(self) -> Task | None:
        ...
        for task in self._queue:
            if task.status == JobStatus.PENDING and not task.cancel_flag.is_set():
                return task
        return None

    def _run_task(self, task: Task) -> None:
        print(f"[WorkQueue] ▶️ Running: [{task.task_id}] {task.goal[:60]}")
        try:
            ...
            if task.on_complete and not task.cancel_flag.is_set():
                try:
                    ...
                    print(f"[WorkQueue] ⚠️ on_complete callback error: {e}")

            print(f"[WorkQueue] ✅ Completed: [{task.task_id}]")
            ...
            with self._lock:
                ...
                task.error  = str(e)
                self._active_count -= 1
            print(f"[WorkQueue] ❌ Failed: [{task.task_id}] {e}")
