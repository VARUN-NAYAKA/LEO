...
GOAL_DECOMPOSITION_PROMPT = """You are the planning module of LEO, a personal AI assistant.
...
def create_plan(goal: str, context: str = "") -> dict:
    ...
    model = genai.GenerativeModel(
        ...
        system_instruction=GOAL_DECOMPOSITION_PROMPT
    )
    ...
    try:
        ...
        for step in plan["steps"]:
            if step.get("tool") in ("generated_code",):
                print(f"[Decomposer] ⚠️ generated_code detected in step {step.get('step')} — replacing with web_search")
                desc = step.get("description", goal)
                step["tool"] = "web_search"
                step["parameters"] = {"query": desc[:200]}

        print(f"[Decomposer] ✅ Plan: {len(plan['steps'])} steps")
        for s in plan["steps"]:
        ...
        print(f"[Decomposer] ⚠️ JSON parse failed: {e}")
        return _fallback_plan(goal)
    except Exception as e:
        print(f"[Decomposer] ⚠️ Planning failed: {e}")
        return _fallback_plan(goal)


def _fallback_plan(goal: str) -> dict:
    print("[Decomposer] 🔄 Fallback plan")
    return {
        "goal": goal,
        "steps": [
            {
...
Create a REVISED plan for the remaining work only. Do not repeat completed steps."""
    ...
    try:
        ...
        print(f"[Decomposer] 🔄 Revised plan: {len(plan['steps'])} steps")
        return plan
    except Exception as e:
        print(f"[Decomposer] ⚠️ Replan failed: {e}")
        return _fallback_plan(goal)