"""
browser_control.py  —  LEO Browser Action Module
====================================================
...
def _real_profile_dir(browser: str) -> str:
    ...
    for p in candidates:
        if p.exists():
            print(f"[WebDriver] ✅ Real profile found for {browser}: {p}")
            return str(p)

    fallback = home / ".leo_profiles" / browser
    fallback.mkdir(parents=True, exist_ok=True)
    print(f"[WebDriver] ⚠️  Real profile not found for {browser}, using: {fallback}")
    return str(fallback)
...
def _firefox_profile_dir() -> Optional[str]:
    ...
    if default_path and Path(default_path).exists():
        print(f"[WebDriver] Firefox real profile: {default_path}")
        return default_path
    return None
...
def _find_opera_windows() -> Optional[str]:
    ...
    for p in candidates:
        if p.exists():
            print(f"[WebDriver] Opera found at: {p}")
            return str(p)
            ...
            for hive in (winreg.HKEY_LOCAL_MACHINE, winreg.HKEY_CURRENT_USER):
                try:
                    ...
                    if exe and Path(exe).exists():
                        print(f"[WebDriver] Opera found via registry: {exe}")
                        return exe
                except Exception:
                    continue
    except Exception:
...
def _resolve_browser(name: str) -> dict | None:
    ...
    if spec.get("special") == "opera_windows":
        ...
        if not exe:
            print(f"[WebDriver] ⚠️  Opera executable not found on Windows.")
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".leo_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[WebDriver] Firefox real profile failed ({e}), using LEO profile")
                LEO = str(Path.home() / ".leo_profiles" / "firefox_LEO")
                Path(LEO).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(LEO, **kwargs)

            await asyncio.sleep(0.5)  # let the browser settle
            # Always open a dedicated LEO tab instead of grabbing pages[0],
            # which may already be about:blank or a stale restored tab.
            self._page = await self._context.new_page()
            print(f"[WebDriver] ✅ Firefox launched")
            return

        # ── Safari (webkit) ───────────────────────────────────────────────────
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".leo_profiles" / "safari")
            ...
            self._page = await self._context.new_page()
            print(f"[WebDriver] ✅ Safari launched")
            return
        ...
        try:
            ...
            self._page = await self._context.new_page()
            print(f"[WebDriver] ✅ Launched [{label}] profile={profile}")
            return
        except Exception as e:
            print(f"[WebDriver] ⚠️  Real profile failed for {label}: {e}")

        # 2) LEO fallback profili
        LEO_profile = str(Path.home() / ".leo_profiles" / self.browser_name)
        Path(LEO_profile).mkdir(parents=True, exist_ok=True)
        print(f"[WebDriver] Retrying with LEO profile: {LEO_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(LEO_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            print(f"[WebDriver] ✅ Launched [{label}] with LEO profile")
    ...
    async def go_to(self, url: str) -> str:
        ...
        • On PlaywrightTimeout we check whether the URL actually changed;
          if it did, treat it as success so LEO won't retry endlessly.
        ...
        async def _do_goto(p: Page) -> str:
            ...
            try:
                ...
                print(f"[WebDriver] goto exception (non-fatal): {e}")
            return p.url
        ...
        if result_url in ("about:blank", "", None, prev_url) and prev_url in ("about:blank", "", None):
            print(f"[WebDriver] Still blank after goto — retrying on new tab: {url}")
            try:
                new_page   = await self._context.new_page()
                self._page = new_page
                result_url = await _do_goto(new_page)
            except Exception as e:
                print(f"[WebDriver] New-tab retry failed: {e}")
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "leo_screenshot.png")
